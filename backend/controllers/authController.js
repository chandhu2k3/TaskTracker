...
const verifyEmail = async (req, res) => {
  try {
    ...
    // Strip any accidental path suffix from env var (e.g. /dashboard)
    const FRONTEND_URL = (process.env.FRONTEND_URL || "http://localhost:3000")
      .replace(/\/(dashboard|verify-email).*$/, "")
      .replace(/\/+$/, "");

    const user = await User.findOne({ verificationToken: token }).select(
      "+verificationToken +verificationTokenExpiry +emailVerified",
    );
    if (!user) return res.redirect(`${FRONTEND_URL}/login?verified=invalid`);
    if (user.verificationTokenExpiry < new Date())
      return res.redirect(
        `${FRONTEND_URL}/login?verified=expired&email=${encodeURIComponent(user.email)}`,
      );
    ...
    res.redirect(
      `${process.env.FRONTEND_URL || "http://localhost:3000"}/login?verified=error`,
    );
  }
};
